...
class UserController {
    ...
    login(req, res) {
        ...
        this.db.collection(USERS).findOne({
            ...
            if (_user.isActivated === false) {
                ...
                var tokenManager = new TokenManager_1.TokenManager(this.db);
    ...
    sendActivationEmail(activationCode, user) {
    ...
    createUser(req, res) {
        ...
        this.db.collection(USERS).findOne({
            ...
            var activationCode = CryptoHelper_1.CryptoHelper.encrypt(user.email + '~' + activationExpiry.getTime().toString());
            if (!_user) {
    ...
    forgotPassword(req, res) {
        ...
        this.db.collection(USERS).findOne({
            ...
            if (!_user) {
                ...
                var resetCode = CryptoHelper_1.CryptoHelper.encrypt(email + '~' + resetExpiry.getTime().toString());
    ...
    validateResetPasswordCode(req, res) {
    ...
    resetPassword(req, res) {
        ...
        if (payload.resetCode != null) {
            ...
            if ('email' in payload == false) {
                ...
                this.db.collection(USERS).update({ email: payload.email }, { $set: { password: payload.password } })
                    .then((_resp) => {
                    if (_resp.result.n != 0) {
